document.addEventListener('DOMContentLoaded', async function() {
    ...
    async function init() {
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(async (position) => {
                ...
                // Calculate the closest city in a parallelized manner
                const closestCity = await getClosestCityParallel(lat, lon, citiesData);

                // Call startApp with the closest city
                startApp(closestCity);
            }, () => {
                // Handle case where location access is denied
                const defaultCity = 'Los Angeles'; // Default city if location access is denied
                startApp(defaultCity);
            });
        } else {
            console.log('Geolocation is not supported by this browser.');
            const defaultCity = 'Los Angeles'; // Default city if geolocation is not supported
            startApp(defaultCity);
        }
    }

    // Function to start the application with the closest city
    ...
    function populateVenueSearch(venues) {
     ...
     // Function to calculate the haversine distance for a single city
     async function calculateDistance(lat1, lon1, city) {
        const R = 6371; // Earth's radius in kilometers

        // Convert latitude and longitude from degrees to radians
        const lat1Rad = deg2rad(lat1);
        const lon1Rad = deg2rad(lon1);
        const lat2Rad = deg2rad(city.latitude);
        const lon2Rad = deg2rad(city.longitude);
    ...
    // Function to find the closest city in a parallelized manner
    async function getClosestCityParallel(lat, lon, citiesData) {
        // Use Promise.all to calculate distances concurrently
        const distancePromises = citiesData.map(city =>
            calculateDistance(lat, lon, city)
                .then(distance => ({ city: city.greaterCity, distance }))
        );

        const distances = await Promise.all(distancePromises);

        // Find the closest city based on the calculated distances

document.addEventListener('DOMContentLoaded', async function() {
    ...
    async function init() {
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(async (position) => {
                ...
                currentCity = await findClosestGreaterCity(lat, long, collection); // Pass the collection
    ...
    function populateVenueSearch(venues) {
      ...
      async function findClosestGreaterCity(userLatitude, userLongitude, collection) {
        // Perform a geospatial query using MongoDB's $geoNear aggregation stage
        const result = await collection.aggregate([
            {
                $geoNear: {
                    near: {
                        type: 'Point',
                        coordinates: [userLongitude, userLatitude], // User's longitude and latitude
                    },
            ...
            {
                $sort: { distance: 1 }, // Sort the results by distance
            },
            {
                $limit: 1, // Get the closest city
            },
            {
                $project: {
                    greaterCity: 1, // Include only the 'greaterCity' field in the result
                    _id: 0, // Exclude the document ID from the result
                },
            },
        ]).toArray();
    
        // Check if there's a result and return the closest greaterCity, or a default value if no result is found
        return result.length > 0 ? result[0].greaterCity : 'Los Angeles'; // Default to Los Angeles if no result
    }
